...
public class AI {
    ...
    private static boolean checkPlaceable(int i, int j, ChessPiece currentPlayer) {
        if (chessGrids[i][j].getChessPiece() == ChessPiece.BLACK || chessGrids[i][j].getChessPiece() == ChessPiece.WHITE)
            return false;
        if (GameFrame.AICheat) return true;
        for (int T = 0; T < 8; T++) {
            if (panel.canPut(i + UndoList.xDirection[T], j + UndoList.yDirection[T], T, currentPlayer, false, chessGrids))
                return true;
        }
        return false;
    }
